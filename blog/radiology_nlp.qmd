...
image-alt: "Flow chart showing the NLP flow for radiology reports. The flow is radiology reports -> entity detection -> context interpretation -> structured output."
subtitle: >
execute:
    eval: false
---
...
![NLP entity extraction pipeline](img/abubakar_1.png){#fig-pipeline fig-alt="Flow chart showing the NLP flow for radiology reports. The flow is radiology reports -> entity detection -> context interpretation -> structured output."}
...
data <- data.frame(
  ...
  Description = c(
    "Unique sequential identifier for each extracted entity.",
...
example <- data.frame(
  entity_text = c("consolidation", "pleural effusion", "pneumothorax"),
...
Full code is [publicly available in my GitHub account](https://github.com/bashir-abubakar/radiology-nlp-r).