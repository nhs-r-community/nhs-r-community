...
The {[tidyverse}](https://www.tidyverse.org/) collection of R packages is a useful set of tools for data analysis and visualisation that are designed to work together. It contains the [{ggplot}](https://ggplot2.tidyverse.org/) package, which we use for visualisations. (Need help getting started with R and the {tidyverse}? Try the [R for Data Science](https://r4ds.hadley.nz/) website).
...
library(tidyverse)
library(readxl)
# library(lubridate) Now in the {tidyverse} package
library(broom)
library(geojsonio)
# library(maptools) # Removed from CRAN 2023-10-16
```
...
# starts with the date column, we lost the few columns that contain trust code, name etc. in line 2
...
import_sitrep <- function(file, indicator) {
  ...
  # Based on the first line, the function exptects dates and
...
# Coded to na using {tidyverse} functions and then plotted - edit 20240612
...
! `tidy.SpatialPolygonsDataFrame()` was deprecated in broom 1.0.4 and is now defunct.
ℹ Please use functions from the sf package, namely `sf::st_as_sf()`, in favor of sp tidiers.
:::
...
```{r}
# to use ggplot for maps, we first need to turn sp data into a data frame
...
map_monthly_bedocc <- ggplot() +
  ...
  theme(
    plot.title = element_text(size = 25 * ggplot2:::.pt, colour = "#005078", margin = margin(b = 15, unit = "mm")),
