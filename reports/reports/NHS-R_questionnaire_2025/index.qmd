...
data <- read_excel("../NHS-R Questionnaire 2025 Data Export.xlsx") 

names(data) <- c("id","userno","name","email","ip","uniid","started","ended","engaged_very","engaged_active","engaged_less","engaged_inactive","engaged_unaware", "why_not_more",
"work_area","career_phase","NHS-R Community website","NHS-R conference – in person attendance","NHS-R conference – online live attendance","NHS-R conference – recordings"," Training – online attendance","Training – YouTube recordings","Coffee and Code ","Webinars – online attendance","Webinars – YouTube recordings","Book: NHS-R Way ","Book: Health Inequalities Notebook","Book: Open Analytics Resources","Book: NHS-R Training","NHS-R GitHub/packages","Blogs","Podcasts","Slack","LinkedIn","Bluesky","Mastodon",
"continue","most_value","value_because","mean_to_you","other_sources","suggestions","testimonial","prize_draw_name","prize_draw_email")

data <- data |> filter(!is.na(id)) |>
  select(-c(userno, name, email, ip, uniid, started, ended)) 
data <- data |>  mutate(career_phase = factor(career_phase, levels = c("Early days (< 1 year)","Beginner (1 - 3 years)","Experienced (3+ years)","Advanced (5+ years)")))
...
data$most_value[data$most_value == "Trainings/webinars online"] <- "Webinars – YouTube recordings"
...
The 2025 NHS-R Community Questionnaire was opened on 2025-04-09 and closed on 2025-05-07 (`r as.Date("2025-05-07") - as.Date("2025-04-09")` days in total) . In total we recieved `r length(data$id)` responses.
